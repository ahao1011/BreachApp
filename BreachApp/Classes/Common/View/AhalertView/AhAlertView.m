...
typedef enum {  // 顶部提示信息的类型
    ...
    AlertViewTypeTf,
    /** textView */
    AlertViewTypeTextView,
    /**协议*/
    AlertViewTypeArge,
    
}AlertViewType;  //  中间弹框的类型

static AlertViewType alertType = AlertViewTypeDefault;
...
@interface AhAlertView ()<UITextFieldDelegate,UITableViewDataSource,UITableViewDelegate,UITextViewDelegate>
...
+ (void)setAlertViewType:(AlertViewType)type{
...
+ (void)alertViewWithtextFieldAndTitle:(NSString *)title message:(NSString *)message OtherMessage:(NSString*)OtherMessage target:(id)target  confirmbuttonTitle:(NSString *)confirmbuttonTitle confirmbuttonAction:(SEL)Action{
    ...
    [AhAlertView setAlertViewType:AlertViewTypeTf];
...
#pragma mark -  二个按钮Block(长文本备用)
- (instancetype)initAWithTitle:(NSString *)title message:(NSString *)message  leftButtonTitle:(NSString *)leftButtonTitle leftButtonHandle:(void(^)())leftHandel rightButtonTitle:(NSString *)rightButtonTitle rightButtonHandle:(void(^)())rightHandel{
...
- (instancetype)initWithTitle:(NSString *)title message:(NSString *)message target:(id)target  leftButtonTitle:(NSString *)leftButtonTitle eftButtonAction:(SEL)leftAction rightButtonTitle:(NSString *)rightButtonTitle rightButtonAction:(SEL)rightAction{
    ...
    CGFloat left = K_Screen_Width * .2 * .5;
    CGFloat messageFont = 14.0f;
    CGFloat width = K_Screen_Width-2*left;
    CGFloat height = 0;
    ...
    switch (alertType) {
        case AlertViewTypeDefault:{
             CGSize size = [message sizeWithFont:[UIFont systemFontOfSize:messageFont] andMaxSize:CGSizeMake(width-4, MAXFLOAT)];
            height = size.height+18+(title==nil?0:40)+40;
        }
            break;
        case AlertViewTypeTf:{
            
            height = 160;
           
        }
            break;
        case AlertViewTypeTextView:{
            height = 250;
        }
            break;
        case AlertViewTypeArge:{
    ...
    self.layer.cornerRadius = 9.5f;
    self.layer.masksToBounds = YES;
    self.alpha = 0.95;
    if (self) {
        // 提示面板
        UIView *alertview = [[UIView alloc]init];
        alertview.layer.cornerRadius = 8.5f;
        ...
        CGFloat mesLabelheight = leftBtn.frame.origin.y - titleLable.frame.origin.y - titleLable.frame.size.height-1;
        
        messageView.frame = CGRectMake(mesLabelX, mesLabelY, mesLabelWidth, mesLabelheight);
        mesLabel.frame = CGRectMake(0, 0, mesLabelWidth, mesLabelheight-3);
...
- (instancetype)initWithTitle:(NSString *)title AttributedString:(NSMutableAttributedString*)AttributedString leftButtonTitle:(NSString *)leftButtonTitle leftButtonHandle:(void(^)())leftHandel rightButtonTitle:(NSString *)rightButtonTitle rightButtonHandle:(void(^)())rightHandel doHandle:(void(^)())doHandel{
    ...
    [AhAlertView setAlertViewType:AlertViewTypeArge];
...
- (instancetype)initWithTitle:(NSString *)title DescString:(NSString*)DescString leftButtonTitle:(NSString *)leftButtonTitle leftButtonHandle:(void(^)())leftHandel rightButtonTitle:(NSString *)rightButtonTitle rightButtonHandle:(void(^)())rightHandel{
    [AhAlertView setAlertViewType:AlertViewTypeTextView];
    self = [[AhAlertView alloc]initAWithTitle:title message:nil leftButtonTitle:leftButtonTitle leftButtonHandle:leftHandel rightButtonTitle:rightButtonTitle rightButtonHandle:rightHandel];
    if (self) {
